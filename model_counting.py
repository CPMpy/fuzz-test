...
def metamorphic_test(solver, iters,f,enb,exclude_dict):
    ...
    with open(f, 'rb') as fpcl:
        ...
        for i in range(iters):
            ...
            try:
                ...
                if isinstance(e,CPMpyException):
                    ...
                    return True
                filename = join(solver + '-counting' + str(iters), "internalfunctioncrash"+str(enb)+".pickle")
        ...
        enb += 1
        with open(join(solver + '-counting' + str(iters), "lasterrormodel" + str(enb)+".pickle"), "wb") as f:
...
if __name__ == '__main__':
    ...
    resultfile = join(solver + '-counting' + str(iters), 'result_counting')
    if not Path(solver + '-counting' + str(iters)).exists():
        os.mkdir(solver + '-counting' + str(iters))
    exclude_dict = {}
    dirname = "models"
    ...
    with open(resultfile, "wb") as ff:
        pickle.dump({'nb_of_models' : nb_of_models, 'hours' : hrs, 'nb_of_errors' : enb, 'solver' : solver, 'testtype' : 'counting', 'iters' : iters, 'randomseed' : rseed}, file=ff)  # log some stats
