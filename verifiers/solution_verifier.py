    ...
    def run(self,solver: str, mutations_per_model: int, model_file: str, exclude_dict: dict, max_duration: float,seed: float) -> dict:
        ...
        try: 
            ...
            with open(model_file, 'rb') as fpcl:
                ...
                Model(cons).solve(time_limit=max_duration-time.time())
                ...
                for i in range(mutations_per_model):
                    ...
                    try:
                        ...
                        return {"type": "internalfunctioncrash","function":function, "argument": argument, "originalmodel": originalmodel, "exception": e, "mutators": mutators,"stacktrace":traceback.format_exc(), "constraints":cons} # no need to solve model we didn't modify..
                ...
                try:
                    ...
                    return {"type": "internalcrash","model": model, "originalmodel": originalmodel, "mutators": mutators,"exception": e,"stacktrace":traceback.format_exc(), "constraints":cons }
    ...
    def rerun(self,solver: str, mutations_per_model: int, seed: float ,error: dict) -> dict:
        try:
            ...
            with open(originalmodel, 'rb') as fpcl:
                ...
                cons = toplevel_list(cons)
                assert (len(cons)>0), f"{originalmodel} has no constraints after l2conj"
                vars = get_variables(cons)
                Model(cons).solve(solver=solver)
                solution = [var == var.value() for var in vars if var.value() is not None]
                mutators = [copy.deepcopy(cons)] #keep track of list of cons alternated with random seed and mutators that transformed it into the next list of cons.
                random.seed(seed)
