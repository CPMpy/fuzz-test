...
class Solver_Vote_Sol_Verifier(Solver_Voting_Verifier):
    """
        The Solver Sol Verifier will verify if a single solution is also in all other solvers after running multiple mutations
    """

    def __init__(self, solver: str, mutations_per_model: int, exclude_dict: dict, time_limit: float, seed: int, mm_prob: float):
        self.name = "solver_vote_sol_verifier"
        self.type = 'sat'
    ...
    def initialize_run(self) -> None:
        ...
        self.og_vars = get_variables(self.cons)
    ...
    def verify_model(self, is_bug_check=False) -> None | dict:
        try:
            ...
            rand_solver = random.choice(self.solvers)
            model.solve(solver=rand_solver, time_limit=time_limit)
            sol = [var == var.value() for var in self.og_vars if var.value() is not None]
            model += sol
