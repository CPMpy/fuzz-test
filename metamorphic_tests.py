...
def metamorphic_test(solver, iters,f,enb,consper, exclude_dict):
    # list of mutators.
    ...
    with open(f, 'rb') as fpcl:
        ...
        for i in range(iters):
            ...
            try:
                ...
                filename = join(solver + '-metamorphic' + str(iters),"internalfunctioncrash"+str(enb)+".pickle")
        ...
        enb += 1
        with open(join(solver + '-metamorphic' + str(iters),"lasterrormodel" + str(enb)+".pickle"), "wb") as f:
    ...
    while time.time() < endtime:
        ...
        for fmodel in fmodels:
            ...
            if time.time() > endtime:
                break
            sat = metamorphic_test(solver, iters, fmodel, enb, consper, exclude_dict)
            if not sat:
                enb += 1
            nb_of_models += 1

    with open(resultfile, "wb") as ff:
        pickle.dump({'nb_of_models' : nb_of_models, 'hours' : hrs, 'nb_of_errors' : enb, 'solver' : solver, 'testtype' : 'metamorphic_tests', 'iters' : iters, 'randomseed' : rseed}, file=ff)  # log some stats
