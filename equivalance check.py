...
def metamorphic_test(solver, iters,f,enb,exclude_dict):
    ...
    with open(f, 'rb') as fpcl:
        ...
        for i in range(iters):
            ...
            try:
                ...
                if isinstance(e,CPMpyException):
                    ...
                    return True
                filename = join(solver + '-equivalence'+str(iters), "internalfunctioncrash"+str(enb)+".pickle")
        ...
        enb += 1
        with open(join(solver + '-equivalence'+str(iters), "lasterrormodel" + str(enb)+".pickle"), "wb") as f:
...
if __name__ == '__main__':
    ...
    resultfile = join(solver + '-equivalence'+str(iters), 'result')
    if not Path(solver + '-equivalence'+str(iters)).exists():
        os.mkdir(solver + '-equivalence'+str(iters))
    exclude_dict = {}
    dirname = "models"
    ...
    with open(resultfile, "wb") as ff:
        pickle.dump({'nb_of_models' : nb_of_models, 'hours' : hrs, 'nb_of_errors' : enb, 'solver' : solver, 'testtype' : 'equivalence', 'iters' : iters, 'randomseed' : rseed}, file=ff)  # log some stats
