...
def metamorphic_test(solver: str, iters: int, model_file: str, exclude_dict: dict) -> dict:
    """
    A helper function that will execute a single model counting test
...
def model_counting_tests(test_results: dict, current_amount_of_tests, current_amount_of_error, lock, solver: str, iters: int, folders: list, max_error_treshold: int, rseed: int) -> None:
    """
        A function that will execute multiple model counting test
    ...
    try:
        ...
        while current_amount_of_error.value < max_error_treshold:
            ...
            for fmodel in fmodels:
                ...
                lock.acquire()
                try:
                    test_results["model_counting_tests"] = {'amount_of_tests': amount_of_tests,'nb_of_models' : nb_of_models, 'nb_of_errors' : len(errors), 'solver' : solver, 'iters' : iters ,"errors" :errors}
